 ...
 *
 * Props:
 * @param {Array} initialTasks - Array of task objects (id, column, tag, title). Default: demo tasks.
 * @param {Array} columns - Array of column objects (id, title, canAdd, isTrash). Default: demo columns.
 * @param {function} onTasksChange - Callback when tasks change (tasks: Array) => void
 * @param {function} onTaskMove - Callback when a task is moved (task, toColumnId) => void
 * @param {function} onTaskAdd - Callback when a task is added (task) => void
 * @param {function} onTaskDelete - Callback when a task is deleted (task) => void
 * @param {string} className - Additional class names for the board container.
 * @param {object} style - Inline styles for the board container.
 */
...
const DEMO_TASKS = [
  {
    ...
    column: "backlog",
    tag: "SYSTEM",
    title: "Optimize oklab border rendering for high-density displays",
  },
  {
    id: "2",
    column: "backlog",
    tag: "CORE",
    title: "Refine jjannon swash alternates in editorial blocks",
  },
  {
    id: "3",
    column: "backlog",
    tag: "VISUAL",
    title: "Audit atmospheric shadow bleed on modal transitions",
  },
  {
    id: "4",
    column: "todo",
    tag: "LOGIC",
    title: "Implement staggered motion primitives for list entry",
  },
  {
    id: "5",
    column: "todo",
    tag: "DB",
    title: "Normalize schema for collaborative real-time editing",
  },
  {
    id: "6",
    column: "in-progress",
    tag: "PERF",
    title: "Calibrate layout engine for oklab-consistent depth",
  },
  {
    id: "7",
    column: "in-progress",
    tag: "DEVEX",
    title: "Inject visual tokens into downstream build pipeline",
  },
  {
    id: "8",
    column: "in-progress",
    tag: "UI",
    title: "Draft secondary pill variations for metadata tags",
  },
  {
    id: "9",
    column: "complete",
    tag: "SHIPPED",
    title: "Configure tailwind oklab color primitives",
  },
  {
    id: "10",
    column: "complete",
    tag: "SHIPPED",
    title: "Establish 8px grid baseline spacing rhythm",
  },
];
...
export const KanbanBoard = ({
  ...
  const handleDrop = (e, targetColumnId) => {
    ...
    if (targetColumnId === "trash") {
      setTasks((prev) => {
        const updated = prev.filter((t) => t.id !== draggedTaskId);
        if (onTaskDelete) onTaskDelete(draggedTask);
      ...
      setTasks((prev) => {
        ...
        if (onTaskMove) onTaskMove(draggedTask, targetColumnId);
        if (onTasksChange) onTasksChange(updated);
        return updated;
      });
    }
