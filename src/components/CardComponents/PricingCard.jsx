...
const PricingCard = () => (
  <div className="w-full flex flex-col items-center py-8 px-4">
    {/* Header */}
    <h2 className="text-[40px] md:text-[52px] text-[#11100a] font-medium tracking-tight text-center mb-4 transition-colors">
      Engineered for <span className="italic font-serif text-[#cf2d56]">Craft.</span>
    </h2>
    <p className="text-[#48473f]/60 text-center text-[16px] md:text-[18px] leading-relaxed max-w-2xl mb-12 transition-colors">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-4xl">
      {PLANS.map((plan, i) => (
        <motion.div
          ...
          className={`relative bg-[#fdf8f7] rounded-xl p-7 flex flex-col justify-between transition-all ${
            plan.popular 
        ...
        >
          ...
          {/* Plan info */}
          <div>
            <h3 className="text-[18px] font-medium text-[#11100a]/80 mb-3">{plan.name}</h3>
            <div className="flex items-baseline gap-1 mb-6">
              <span className="text-[42px] font-medium text-[#11100a] tracking-tight leading-none">{plan.price}</span>
              {plan.period && <span className="text-[16px] text-[#48473f]/40">{plan.period}</span>}
            </div>

            {/* Features */}
            <div className="flex flex-col gap-3 mb-8">
              {plan.features.map((f, j) => (
                <div key={j} className={`flex items-center gap-2.5 text-[15px] ${f.active ? 'text-[#11100a]/70' : 'text-[#48473f]/40'}`}>
                  {f.active 
                    ? <CircleCheck size={16} className="text-[#cf2d56]/70 shrink-0" /> 
                    : <Lock size={14} className="text-[#48473f]/20 shrink-0" />}
          ...
          <motion.button
            ...
            className={`w-full py-3 rounded-lg text-[15px] font-medium transition-colors cursor-pointer ${
              plan.popular
                ? 'bg-[#11100a] text-[#ffffff] hover:bg-[#cf2d56]'
                : 'bg-transparent oklab-border text-[#11100a]/60 hover:bg-[#1c1b1b]/5'
            }`}
          >
            {plan.cta}
          </motion.button>
